...
| 状态 | 切片 |
| --- | --- |
| 已合并 | S1–S27，含补记的 S19.5（文字颜色与背景色，实际早于 S19 合入）。S12 的演示服务与可替换服务契约位于 `apps/remote-image-service` 和 `packages/editor-remote-image-service`。S13 的 y-prosemirror 前置验证已记录在 `docs/y-prosemirror-compatibility.md`。 |
| 待决策 | 无。 |
...
| M4：协同、评论、版本历史、AI | §17 | 已拆成 S28–S31 的切片规格（见下），未实现。 |
...
方案 §17 的 M4 是四个能力，之前只在里程碑表里有一行字。这里把它们拆成有编号、有验收、有回滚的切片，好让"什么时候能开工、开工要动哪些不可逆的东西"变成可以判断的问题。**下面四片都还没写一行代码。**

顺序不能随意换：S28 会改历史实现，S29、S30 都要在它定下的位置语义上建，AI 是唯一可以并行的一片。

### S28. Yjs 协同 · 未认领

- **动什么**：`editor-pm-adapter` 增加协同会话配置——安装 `ySyncPlugin(fragment)`，把 `recordHistory` 的实现从 `prosemirror-history` 换成 `Y.UndoManager`；`provider` 契约（连接、断连、重连、awareness）定义在 adapter，具体实现（WebSocket / WebRTC / 自建）由宿主注入；光标与选区的 awareness 渲染走 Decoration。
- **前置已完成**：y-prosemirror 兼容性验证见 `docs/y-prosemirror-compatibility.md`，结论是接入面可以限制在 `editor-pm-adapter`，业务 API 与 Schema 不动。
- **最大的坑，先写在这里**：那份 PoC 用的是内存 Y.Doc 与 jsdom，**没有覆盖 provider 断连重连、awareness、两个真实浏览器的并发编辑，以及中文组合态期间远端事务的排队**。最后一条尤其危险——远端事务和 §9.6 的挂起队列是同一个问题的两半，而 S25 刚证明了这条路径在 jsdom 里验不出来。这一片的验证必须从两个真实浏览器开始，不是从单测开始。
- **未知节点的空白**：当前 `unknown_*` 兜底只覆盖平台 JSON 装载，**对 `Y.XmlFragment` 的未知节点降级尚未实现也未验证**。在补上之前，缺插件的客户端不得加入协作文档——否则 §9.3 那条"缺插件不丢内容"的承诺在协同场景下是空的。这是本片的第一项工作，不是后续优化。
- **演示**：两个浏览器窗口打开同一文档，并发编辑同一段落不冲突；一方断网继续编辑、恢复后合并无丢失；看得见对方的光标与选区。
- **验证**：Playwright 双上下文并发编辑（这正是 S25 的浏览器验收基建存在的意义）；中文输入法组合期间收到远端事务不断字；`Y.UndoManager` 下的撤销只回退自己的改动。
...
- **动什么**：消费 §9.8 已经冻结的锚点模型——评论存在信封的 `annotations` 里，**不做 mark**。`Annotation` 已有 `{id, from, to, orphaned, payload}` 字段；这一片补锚点随事务映射（`tr.mapping.map`，与 §9.5 的图片上传同一套机制）、锚点目标被删除时置 `orphaned` 而不是丢弃、评论高亮用 Decoration 渲染、`comment.*` 命令与侧栏 UI。
- **为什么锚点不做 mark**（§9.8 已定论，这里只是提醒别在实现时走回头路）：mark 会随文本分裂合并产生 ID 去重问题，删除评论会留下残留 mark，还会污染复制粘贴与导出。
- **与 S28 的关系**：单机评论可以先做，但锚点在协同下要迁移为 `Y.RelativePosition`（§19 第 6 条）。**先做单机版就等于承诺一次迁移**——建议排在 S28 之后，除非有明确的单机先行需求。
- **演示**：选中一段文字加评论 → 侧栏出现 → 在评论前面插入大段文字，高亮跟着走 → 删掉被评论的文字，评论变成"内容已删除"而不是消失。
- **验证**：锚点映射的属性测试（随机编辑序列后锚点仍指向同一段文字）；`orphaned` 转换；评论不进 `getHTML()` / `getMarkdown()`（它是元数据不是正文）。
- **回滚**：单 commit；回滚后 `annotations` 字段仍在信封里原样留存，评论 UI 消失但数据不丢。
- **依赖**：S28（若要协同下可用）。
- **PRD**：§9.8、§19 第 6 条

### S30. 版本历史 · 未认领

- **动什么**：消费 S14 的 `DocumentPatch` 流——服务端按 patch 累积版本，客户端提供"查看某个版本""对比两个版本""恢复到某个版本"。恢复实现为**追加一笔把内容改回去的新变更**，不是回退历史指针。
- **为什么恢复要追加而不是回退**：协同下没有"回退"这回事——别人的后续编辑还挂在后面。追加一笔反向变更是唯一能和 S28 共存的语义，而且它自身也可被撤销。
- **前置约束**：`DocumentPatch` 的位置语义（文档扁平偏移量）是 §4 里的不可回滚边界。这一片开始之前，patch 历史的存储格式要一次定对。
- **演示**：编辑若干次 → 打开历史面板看到时间轴 → 选两个版本看差异 → 恢复到较早一版，当前文档变回去，且这次恢复自己也在历史里。
- **验证**：属性测试（任意编辑序列的 patch 重放结果与直接编辑等价——S14 已有，这里扩展到跨版本区间）；恢复后的文档与目标版本全等；恢复可被撤销。
- **回滚**：单 commit；回滚后历史面板消失，已存的 patch 流不受影响。
- **依赖**：S14、S28（协同下的版本语义）。
- **PRD**：§8.4、§12.3、§17

### S31. AI 能力 · 未认领

- **动什么**：AI 改写 / 续写 / 摘要。**这一片的技术难点不是模型调用，是位置。** AI 请求发出到结果返回之间用户还在编辑，返回的位置一定是过期的——必须完整走 §9.5 的异步契约：请求态存 plugin state 不进文档、每个事务用 `tr.mapping.map()` 迁移位置、用 Decoration 渲染"正在生成"、回填事务 `recordHistory:false`、目标位置消失时丢弃结果。与图片上传（S11）是同一套机制，不该另造一套。
- **组合态**：AI 回填是典型的"程序化事务"，必须遵守 §9.6 的挂起队列。S25 的用例已经证明这条路径在 jsdom 里验不出来，AI 回填要在真实浏览器里验一遍。
- **可与 S28–S30 并行**：它只依赖 S11 定下的异步契约，不依赖协同或历史。
- **演示**：选中一段 → "改写" → 占位出现 → 期间在前面插入文字 → 结果回填到正确位置；生成中撤销删除选区，结果到达时被丢弃。
- **验证**：与 S11 同构的单测（mapping 迁移、丢弃、`recordHistory:false`）；真实浏览器下组合态期间回填被挂起。
...
| Yjs 更新历史成为事实来源 | S28（未实现） | 退回 `prosemirror-history` 等于丢弃全部协作历史 |

其余切片在有真实数据后仍可安全 revert：功能消失，但已有文档因 S2 兜底不丢内容。**这就是 S2 必须排在所有 `co_` 节点之前的原因。**

S28 是这张表上唯一还没做的一条。它的判定时点不是"上线协同"，而是"第一份协作文档落库"——在那之前换实现只是改代码，在那之后是丢数据。
...
| M4 高级能力 | S28 – S31（见 §3.1，已定规格、未实现）。入口是 S14 的 patch 流与 S1 的 `annotations` 字段 |
