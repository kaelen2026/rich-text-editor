...
- **动什么**：基准文档生成器（5 万字 / 300 段 / 50 图 / 20 表 / 4 层列表）；五项指标测量（首次可编辑、按键到重绘 p95、粘贴 1 万字、工具栏状态更新、内存增量）；`getDocument()` 按事务缓存；字数统计增量维护；NodeView 懒挂载；CI 超预算 20% 失败。
- **演示**：`pnpm bench` 输出五项指标与预算对照表；故意去掉 `getDocument()` 缓存 → CI 红。
- **验证**：基准脚本本身可重复（同机三次波动 < 10%）；CI 门禁生效。
