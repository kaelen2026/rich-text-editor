...
表里每条记录是 `{id, from, to, payload}`，`from`/`to` 是 `Y.encodeRelativePosition` 的字节。两端偏向与单机 `mapRange` 的语义一致：`from` 锚区间**第一个字符**（assoc 0），`to` 锚**最后一个字符**（assoc -1）——贴着批注外侧输入的文字留在批注之外，端点字符被删时位置坍缩到删除点。`orphaned` 在协同下不落库、由重投影推导：两端撞在一起即孤儿，撤销那次删除会让评论复活；单机下扁平位置无从复原，orphaned 不可逆。这是方向性的差异，不是缺陷。
