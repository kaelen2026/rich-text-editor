...
**S30 补记（patch 历史的存储格式与恢复语义，实现时定的）**：版本历史的持久化容器是 `VersionLog = {v: 1, baseRevision, baseDoc, entries: [{patch, meta?}]}`（`editor-shared-types`）——一份基线快照加修订号连续的变更流，`meta` 是宿主自有的不透明元数据（时间、作者、标签），编辑器不生成也不解释。追加带连续性校验；定期快照与压缩不改这个形状，以新基线另起一份日志即可。查看与对比版本是对日志的纯重放（`documentAtRevision`，服务端可用），不经过编辑器；**恢复是追加一笔把内容改回去的新变更**（`version.restore`），反向变更取自每条 patch 自带的 `inverse` 从新到旧逆放——最小 diff 而非整篇替换，未被这段历史动过的内容一步不挪，压在上面的批注锚点（§9.8）原样存活；派发前与目标版本全等校验，对不上拒绝不动文档。协同下恢复就是一次普通编辑，与 §17 的合并规则共存，且自身可被撤销。
...
第 5 条的判据是**这次异步操作在文档里留没留下中间态**，S31 实现时才把它说清楚：

- 留了（图片上传：插入占位图那一笔已经进历史），回填只是把占位换成成品，再记一次会让撤销停在 loading 态，因此 `recordHistory: false`。
- 没留（AI 改写：生成期间文档一字未动），回填就是用户看到的那次编辑本身。此时仍然 `recordHistory: false` 等于让这次编辑永远撤不回去——必须记。

换句话说这条规则约束的是"别为一个已经在文档里的中间态再记一次"，不是"异步回填一律不进历史"。
...
**S29 补记（协同下评论存哪，实现时定的）**：协同绑定后，批注的权威是**同一个 `Y.Doc` 里的顶层 `Y.Map("annotations")`**，与正文的 XmlFragment 并列，不开第二条同步通道。理由有三：

1. **同一笔更新到达。** 批注和它锚定的文字共享一份更新历史，不存在"文字到了、评论还在路上"的中间态；第二条通道必须自己解决这个排序问题，而它无解。
2. **准入闸门天然放行，这是"评论不做 mark"的第一次真正兑现。** §9.4 的闸门只认更新里的节点名与标记名；纯 JSON 的 `Y.Map` 项两者都不带。没装评论插件的协作者收到评论更新时**原样保存、只是看不见**——同一件事若做成 mark，缺插件的一端会替所有人删掉整段文字。
3. **传输层零改动。** provider 契约（§17）本来就把 `Y.Doc` 摆在接口上。

表里每条记录是 `{id, from, to, payload}`，`from`/`to` 是 `Y.encodeRelativePosition` 的字节。两端偏向与单机 `mapRange` 的语义一致：`from` 锚区间**第一个字符**（assoc 0），`to` 锚**最后一个字符**（assoc -1）——贴着批注外侧输入的文字留在批注之外，端点字符被删时位置坍缩到删除点。锚定前选区两端会各自收进**紧邻字符的文本位置**：终点贴在下一个块开头（那一块一个字没选）时收回上一块末尾，否则锚点会落在块元素上、把整个没选中的块圈进批注；收完两端撞在一起（例如只选中一个原子节点）就不是可锚定的批注目标，命令返回 invalid。`orphaned` 在协同下不落库、由重投影推导：两端撞在一起即孤儿，撤销那次删除会让评论复活；单机下扁平位置无从复原，orphaned 不可逆。这是方向性的差异，不是缺陷。

Yjs 与 y-prosemirror 的知识收在 `editor-pm-adapter`（桥接层，§7.1）；评论插件与业务 API 只见扁平的 `Annotation`。宿主读批注走 `getAnnotations()`：引用在批注未变时稳定，可直接喂给框架订阅。绑定共享文档时本地批注不迁移进批注表——与"绑定后 `loadDocument` 被拒"同一条立场：共享文档才是事实来源，本地正文被替掉了，锚在上面的批注跟着作废。

一个前提这里说破：这套设计下评论与正文同权限——能看到文档的人就能看到全部评论。若将来出现"能看正文但不能看某些评论"的角色，服务端无法从同一条更新流里把批注过滤掉，那时评论必须搬去自己的通道，代价是自己解决上面第 1 条。
