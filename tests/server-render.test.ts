...
/**
 * 同一份 fixture 的 Markdown。颜色、对齐、图片的展示属性和单元格合并在
 * Markdown 里没有写法，因此只剩文字与结构——丢格式不丢内容（方案 §4.3）。
 * 图片本身有写法，导出成 `![alt](src)`。
 */
const expectedMarkdown = `## 服务端渲染

**粗体**和[链接](https://example.com/docs)，还有彩色文字

| 标题 | 数值 |
| --- | --- |
| 第一行 | 42 |

![示例图片](https://cdn.example.com/image.png)
`;
...
describe("服务端 Markdown 导出", () => {
  it("同一份 Schema 在纯 Node 环境导出 Markdown", () => {
    const editor = createFullEditor();
    expect(editor.loadDocument(JSON.parse(readFileSync(fixturePath, "utf8"))).ok).toBe(true);
    expect(globalThis.document).toBeUndefined();
    expect(editor.getMarkdown()).toBe(expectedMarkdown);
  });

  it("render CLI 的 --format markdown 与进程内导出一致", () => {
    expect(
      execFileSync("pnpm", ["--silent", "render", fixturePath, "--format", "markdown"], {
        encoding: "utf8",
      }),
